  ...
  return <section className="setup">
    ...
    <div className="setup-panel">
      ...
      {step === 1 && <>
        ...
        <div className="callout warning"><p>The client secret is a password for this app registration. OneNote Queue sends it to this server over HTTPS, encrypts it, and stores it in your database. Never put it in GitHub, a Shortcut, or a screenshot.</p></div>
      ...
      {step === 2 && <>
        ...
        <p>The button below opens Microsoft&apos;s own sign-in and consent page. OneNote Queue never sees your Microsoft password.</p>
      ...
      {step === 5 && <>
        <p className="availability planned">Under development</p>
        <h2>6. Discord integration is optional and not ready yet</h2>
        <p>The core OneNote connection and Shortcut capture API work without Discord. The Discord interaction endpoint and command registration are not implemented in this release, so there is nothing useful to configure on this screen yet.</p>
        <h3>What the advanced setup will eventually ask for</h3>
        <ul><li><strong>Application ID:</strong> identifies your Discord application.</li><li><strong>Public Key:</strong> lets the server verify that requests really came from Discord.</li><li><strong>Test server (Guild) ID:</strong> optional, for faster command testing.</li><li><strong>Bot token:</strong> only for a future self-hosted gateway mode; it will never be required for Vercel HTTP interactions.</li></ul>
        <div className="callout warning"><p>Do not create or paste a Discord bot token for this version. The site previously showed configuration fields too early; those fields have been removed until the feature can validate and use them.</p></div>
        <p><a href="/docs#advanced-discord">Read the implementation status and planned architecture →</a></p>
      </>}
    </div>
  </section>;
}
