...
export async function GET() {
  try {
    await ensureSchema();
    const rows = await db()`SELECT client_id FROM ons_config WHERE id=1`;
...
export async function POST(req: NextRequest) {
  try {
    await ensureSchema();
    const existing = await db()`SELECT client_id FROM ons_config WHERE id=1`;
    const user = await currentUser();
    ...
    await db()`INSERT INTO ons_config (id, client_id, client_secret_enc, configured_at) VALUES (1, ${clientId.trim()}, ${encrypt(clientSecret.trim())}, now()) ON CONFLICT (id) DO UPDATE SET client_id=excluded.client_id, client_secret_enc=excluded.client_secret_enc, configured_at=now()`;
