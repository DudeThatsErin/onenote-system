...
export async function GET() {
  try {
    ...
    const rows = await db()`SELECT application_id, public_key, bot_token_enc, test_guild_id,
        command_id, registered_scope, registered_at, updated_at
...
export async function POST(req: NextRequest) {
  try {
    ...
    await db()`INSERT INTO ons_discord_config
      (id, application_id, public_key, bot_token_enc, test_guild_id, user_id, updated_at)
      VALUES (1, ${applicationId}, ${publicKey}, ${encryptedToken}, ${testGuildId || null}, ${user.id}, now())
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = now()`;
    return NextResponse.json({ ok: true, hasBotToken: Boolean(botToken) });
  } catch (error) {
