...
export async function accessToken(userId: string) {
  const sql = db();
  const users = await sql`SELECT access_token_enc, refresh_token_enc, expires_at FROM ons_users WHERE id = ${userId}`;
  const user = users[0];
  ...
  await sql`UPDATE ons_users SET access_token_enc=${encrypt(data.access_token)}, refresh_token_enc=${encrypt(data.refresh_token || decrypt(user.refresh_token_enc as string))}, expires_at=${new Date(Date.now() + data.expires_in * 1000).toISOString()}, updated_at=now() WHERE id=${userId}`;
